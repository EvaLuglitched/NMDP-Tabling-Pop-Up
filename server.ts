...
Return ONLY the 3-6 sentence paragraph. No extra markdown headings or bullet points.`;
    ...
    const fallbackSummary = `For the upcoming NMDP Tabling Session, I designed a multi-channel invitation campaign featuring a clean, modern visual poster and an interactive mobile landing page to drive awareness for Blood Cancer and Pediatric Cancer Awareness Month. The invitation featured an encoded QR code deployed across high-traffic Berkeley campus locations, directing students to event logistics outside the Amazon Hub Locker (2495 Bancroft Way) and educational facts regarding blood stem cell donation. Over the campaign tracking period, the invitation generated ${stats.totalVisits} total views and ${stats.qrVisits} direct QR scans, with ${stats.devices.mobile || stats.qrVisits} visits originating from mobile devices on campus. The campaign achieved strong participation with ${stats.totalPledges} students submitting pledges to stop by and get swabbed, while ${stats.events.calendar_add || 0} students synced the tabling session to their calendars. By pairing urgent medical facts—such as a blood cancer diagnosis occurring every 3-4 minutes—with approachable graphic design, the invitation successfully fostered community interest in joining the NMDP Registry.`;
